			...
			go func(server *master.ListTabletServersResponsePB_Entry) {
				...
				tabletReport := &format.Output{
					...
					TableColumns: []format.Column{
						...
						{Name: "START_KEY", Expr: "partition_key_to_hex(@.tablet.tablet_status.partition.partitionKeyStart)"},
						{Name: "END_KEY", Expr: "partition_key_to_hex(@.tablet.tablet_status.partition.partitionKeyEnd)"},
