...
##    Parses the "tablet report" created by yugatool or support-bundle, and creates a sqlite DB.
...
#    Alternatively, the YBA 2025.2+ support bundle can create a tablet report.
...
#
# EXTRAS:
#   * Input files can be gzip compressed (must end with .gz), or a support bundle file or dir.
...
my %opt=(
	...
	SQLITE_ERROR   => (qx|sqlite3 -version 2>&1|=~m/([^\s]+)/  ?  0 : "Could not run SQLITE3: $!"), # Checks if sqlite3 can run
...
our $USAGE = << "__USAGE__";
  ...
  The input to this program is a "$ANSICOLOR{CYAN}tablet report$ANSICOLOR{NORMAL}" created by yugatool or support-bundle.
  ...
  $ANSICOLOR{REVERSE}TYPICAL/DEFAULT/PREFERRED usage:$ANSICOLOR{NORMAL} 
   ...
   $ANSICOLOR{BRIGHT_YELLOW} perl $0 $ANSICOLOR{BRIGHT_CYAN}\[TABLET-REPORT-OR-BUNDLE] \[ENTITY-file] \[tablet-info-file]... $ANSICOLOR{NORMAL}

  This will create and output database file named TABLET-REPORT-FROM-YUGATOOL.sqlite

  $ANSICOLOR{FAINT}ADVANCED usage1: perl $0 TABLET-REPORT | sqlite3 OUTPUT-DB-FILE-NAME$ANSICOLOR{NORMAL}
  $ANSICOLOR{FAINT}ADVANCED usage2: perl $0 [<] TABLET-REPORT > OUTPUT.SQL$ANSICOLOR{NORMAL}
 ...
 $ANSICOLOR{BRIGHT_BLUE}       curl <master-leader-hostname>:7000/dump-entities | gzip -c > $(date -I)-<master-leader-hostname>.dump-entities.gz
 $ANSICOLOR{CYAN}* Files named "<tablet-uuid>.txt"  are assumed to be "tablet-info" files. These are created by:
 $ANSICOLOR{BRIGHT_BLUE}      ./yugatool -m \$MASTERS \$TLS_CONFIG tablet_info \$TABLET_UUID > \$TABLET_UUID.txt $ANSICOLOR{NORMAL}
__USAGE__

if (-t STDIN and not @ARGV){
	if (Handle_support_bundle_directory_or_compressed(".")){
		# The current dir contains a support bundle, so we are OK - fall through
	}else{
...
while (my $inputfilename = shift @more_input_specified){
	# User has specified an argument (Default usage) - we will process it as a filename
	print "SELECT 'Processing $inputfilename....' as info;\n"; 
    if (-d $inputfilename  or  $inputfilename=~/\.tar\.gz$/){
       Setup_Output_Processing($inputfilename);
	   Create_Tables_and_views();
       if (Handle_support_bundle_directory_or_compressed($inputfilename)){
           # We found and processed a support bundle
           next;
        }
        # IF we get here, we did NOT find a Tablet report in support bundle .. fall through for error msg
	}
	...
	if ($inputfilename =~/entities/i){
		...
		Process_tablet_report($inputfilename);
...
#-------------------------------------------------------------------------------------------------
sub Handle_support_bundle_directory_or_compressed{
    my ($file_or_dir) = @_;
    if ($file_or_dir =~/\.tar\.gz$/){
       print "SELECT '$ANSICOLOR{GREEN} $file_or_dir appears to be a support bundle file..$ANSICOLOR{NORMAL}' as INFO;\n";
       # Need to check if already extracted. If so - descend. If not, open the tar.gz.
       return 1; # Successfully processed
    }

    if (-d $file_or_dir){
        opendir(my $dh, $file_or_dir) || die "ERROR:Can't opendir $file_or_dir: $!";
		my $processed_file_count = 0;
        for my $subdir_or_file( readdir($dh) ){
            next if $subdir_or_file =~/^\.|^master$|^tserver$/;
            $processed_file_count+=Handle_support_bundle_directory_or_compressed("$file_or_dir/$subdir_or_file")
        }
        closedir $dh;
    ...
    if (-f $file_or_dir){
        # print "--DEBUG: Found file $file_or_dir.\n";
        if (    $file_or_dir =~/\Wdump-entities\.json$/){
          print "SELECT '   ... processing $file_or_dir as an ENTITIES file..' as info;\n";
          unshift @more_input_specified, $file_or_dir; # Entities to be processed FIRST by main loop
          $opt{JSON} = 1;
          return 1;
        }
        if ($file_or_dir =~ /\Wuniverse-details\.json$/){
          # Handle this case. May not be necessary...
          print "SELECT '   ... processing $file_or_dir as an Univ details file..' as info;\n";
          $universe = UniverseClass::->new($file_or_dir); # Sets up global $universe immediately
		  # At this point, we have NODE UUIDs in the  NODES, not TSERVER UUIDS.
		  # While processing ENTITIES, we will update thse to be tserver_uuids.
          return 1;
        }
        if ($file_or_dir =~/tablet_report\.json$/){
          # Handle this case
          print "SELECT '   ... processing $file_or_dir as an tablet report file..' as info;\n";
          push @more_input_specified, $file_or_dir; # To be processed by main loop
          $opt{JSON} = 1;
          return 1;
        }
...
while ($line=<>){
	if ($opt{JSON}){
	    ...
	    if ($line =~m/^\s?}[\s,]*$/){
		   ...
		   Set_Transaction(1);
		   JSON_Analyzer::Process_line($json_line, $inputfilename);
		   Set_Transaction(0);
...
SELECT 'Tablet report loading Completed.';
__MAIN_COMPLETE__
}
#---------------------------------------------------------------------------------------------
...
my $retry = 10; # Could take 10 sec to generate summary report...
...
sub Setup_Output_Processing{
	...
	$inputfilename=~/\.gz$/i and $inputfilename=substr($inputfilename,0,-3); # Drop the ".gz"
	$inputfilename=~/\.tar$/i and $inputfilename=substr($inputfilename,0,-4); # Drop the ".tar"
    ...
    open ($SQL_OUTPUT_FH, "|-", "sqlite3", $output_sqlite_dbfilename)
         or do {
               warn "ERROR:Could not start sqlite3 :OS error: $!\n";
			   warn "      You can save the generated SQL for later processing by redirecing STDOUT, i.e use '> file.sql'. See --help.\n";
               die  "      Exiting with error - no output produced.";
               };
...
sub new{
  my ($class, $univ_file_name) = @_;
  my $json = entities_parser::->new() ->{JSON}; # get JSON from ent, which imports it for us.
  # Slurp file 
  open my $f,"<", $univ_file_name or die "ERROR: Cannot open $univ_file_name:$!";
  my $contents = do {
    local $/;
    <$f>;
  };
  close $f;
  my $self ={ universeDetails => $json->decode($contents) }; # A proper Universe structure 
...
sub Populate_Cluster_Table{
   my ($self) = @_;
   for my $n (@{ $self->{NODES} }){
      print "INSERT INTO cluster(type, uuid , ip, port, region, zone ,uptime)\n",
	      "  VALUES('TSERVER','",
          join("','", map {$n->{$_}||""} qw| nodeUuid private_ip tserverRpcPort az region 0|),
		  "');\n";
   }
}

} # End of UniverseClass class 
####################################################################################
BEGIN{
...
my %dispatch_by_type = (
	...
	"Tablet report" => sub{Parse_Tablet_line($_[0],$_,$_[1])  for @{$_[0]->{content}}},
);
...
sub Parse_Tablet_line{
	my ($d,$t,$inputfilename) = @_;
	my ($host_name, $host_port,$host_uuid) 
	   = $d->{msg} =~m/\[host:"([^"]+)"\s+port:(\d+).*?\] \((\w+)/;
    if (! $host_uuid){
       # Get uuid from Universe node matching $inputfilename
	   my ($tserver) = grep { $inputfilename =~/$_->{nodeName}/ } @{ $universe->{NODES} };
	   die "ERROR: Could not find tserver for file $inputfilename" unless $tserver;
	   $host_uuid = $tserver->{nodeUuid};
	   TableInfo::->Register_Region_Zone($tserver->{region}, $tserver->{az}, $host_uuid);
	}
	my %values;
