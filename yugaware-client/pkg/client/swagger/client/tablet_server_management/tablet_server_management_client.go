...
func (a *Client) ListTabletServers(params *ListTabletServersParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*ListTabletServersOK, error) {
	...
	if params == nil {
		params = NewListTabletServersParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "listTabletServers",
		Method:             "GET",
		PathPattern:        "/api/v1/customers/{cUUID}/universes/{uniUUID}/tablet-servers",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"http", "https"},
		Params:             params,
		Reader:             &ListTabletServersReader{formats: a.formats},
		AuthInfo:           authInfo,
	...
	success, ok := result.(*ListTabletServersOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for listTabletServers: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
